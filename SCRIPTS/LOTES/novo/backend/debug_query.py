"""Debug para testar a query exata do sistema - corrigido"""
from database import execute_query

# DN Construtora cod: 73309
cliente_cod = 73309

print("=" * 60)
print(f"TESTANDO QUERY PARA CLIENTE {cliente_cod}")
print("=" * 60)
...
SELECT TOP 1 
    ...
    PE.NumEnd_pend as numero_direto,
    PE.Bairro_pend as bairro_direto,
    PE.Cidade_pend as cidade_direto,
    PE.CodEmp_pend as codEmpPend,
    PEmpresa.Nome_pes as empresaVinculada,
    PEComercial.Endereco_pend as endereco_comercial,
    PEComercial.Cidade_pend as cidade_comercial,
...
for r in result:
    ...
    print(f"\n  ENDEREÇO DIRETO (PE):")
    print(f"    {r.get('endereco_direto')}, {r.get('numero_direto')}")
    print(f"    {r.get('bairro_direto')}, {r.get('cidade_direto')}")
    print(f"    CodEmp_pend: {r.get('codEmpPend')}")
    print(f"    Empresa vinculada: {r.get('empresaVinculada')}")
    print(f"\n  ENDEREÇO COMERCIAL (PEComercial):")
    print(f"    {r.get('endereco_comercial')}, {r.get('cidade_comercial')}")
...
# Verificar também a função calcular_valor_quitacao
print("\n" + "=" * 60)
print("VERIFICANDO O HISTÓRICO DE TRANSFERÊNCIAS (calcular_valor_quitacao)")
print("=" * 60)

# Simular a query de calcular_valor_quitacao para o lote Q008 L029
empresa = 6
obra = '70400'
quadra = '008'
lote = '029'
...
SELECT NumVenda, Empresa_Ven, Obra_Ven, Data_Ven, ValorTot_Ven, Cliente, CPF, CodCliente, DataCessao, Origem
FROM VendaAtual
ORDER BY Data_Ven
"""
result_vendas = execute_query(query_vendas)
print(f"\nVendas encontradas (historico_transf): {len(result_vendas)}")
for i, r in enumerate(result_vendas):
    print(f"  {i+1}. Venda {r.get('NumVenda')}: {r.get('Cliente')} (cod: {r.get('CodCliente')}) - Origem: {r.get('Origem')}")

if result_vendas:
    ultimo = result_vendas[-1]
    print(f"\n  ÚLTIMO (cliente atual): Venda {ultimo.get('NumVenda')}, Cliente: {ultimo.get('Cliente')}, Cod: {ultimo.get('CodCliente')}")
